...
const sourceFileUrl = 'docs/newppt-test'
...
co(function* uploadFiles() {
  ...
  const files = glob.sync('**/*', {
    cwd: path.join(__dirname, `../${sourceFileUrl}`),
    nodir: true,
    dot: true,
    ignore: '*index.html',
  })
  ...
  for (let i = 0; i < files.length; i += 1) {
    const file = files[i]
    const key = `${config.uploadDir}/testfile/${file}`
    console.log('key:====', key)
    yield client.put(key, path.join(__dirname, `../${sourceFileUrl}`, file))
